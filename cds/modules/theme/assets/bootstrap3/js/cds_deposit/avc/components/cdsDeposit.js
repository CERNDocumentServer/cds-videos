...
function cdsDepositCtrl(
  ...
  this.$onInit = function () {
    ...
    this.inheritMetadata = function (specific, forceInherit) {
      ...
      angular.forEach(paths, function (propPath) {
        ...
        if (
          (inheritedVal && !ownElement) ||
          (inheritedVal && forceInherit === true)
        ) {
    ...
    this.triggerRestartFlowTask = function (flowId, taskId) {
      that.restartFlowTask(flowId, taskId).then(function () {
    ...
    this.updateDeposit = function (deposit) {
      ...
      that.record._cds.state = angular.copy(deposit._cds.state || {});
    ...
    this.updateSubformatsTranscodingStatus = function (flowTasks) {
      ...
      var subformatsNew = transcodingTasks
    ...
    this.recurrentFetchFlowTasksStatuses = function () {
      let anyTaskIsRunning = that.anyTaskIsRunning();
      if (anyTaskIsRunning) {
        that.fetchFlowTasksStatuses();
      }
    };
    ...
    this.fillMetadata = function (answer) {
      ...
      if (answer) {
        ...
        // Make a partial Save
        that
          .makeSingleAction("SAVE_PARTIAL")
          .then(that.onSuccessAction, that.onErrorAction)
    ...
    this.getMetadataToFill = function (metadata) {
      ...
      Object.keys(depositExtractedMetadata.values).forEach(function (
        item,
        index
      ) {
        var value = depositExtractedMetadata.values[item](
          metadataToFill,
          allMetadata
        );
        if (value) {
  ...
  this.makeSingleAction = function (action) {
    ...
    return cdsAPI.makeAction(url, that.depositType, action, cleanRecord);
  };
