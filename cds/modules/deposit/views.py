...
def assign_report_number(depid):
    ...
    if video.get('report_number'):
        flash('This video already has a report number assigned {0}'.format(
            video['report_number'][0]), category='error')
        return redirect(
            url_for('invenio_deposit_ui.project',
                    pid_value=video.get('_project_id')))


    possible_report_numbers = _get_report_numbers(video)
    form = MintReportNumber()
    form.report_number.choices = [('','')] # Default choice
    form.report_number.choices.extend(
        [(rn, rn) for rn in possible_report_numbers])
    ...
    return render_template('cds_deposit/assign_report_number.html',
                           video=video,
                           form=form)
