...
class Deposit(Record):
    ...
    def build_deposit_schema(self, record):
        ...
        schema_prefix = current_app.config["DEPOSIT_JSONSCHEMAS_PREFIX"]
        if schema_path:
            return current_jsonschemas.path_to_url(schema_prefix + schema_path)
    ...
    def fetch_published(self):
        ...
        resolver = Resolver(
            pid_type=pid_type,
            object_type="rec",
            getter=partial(self.published_record_class.get_record, with_deleted=True),
        )
    ...
    def _publish_new(self, id_=None):
        ...
        """
        minter = current_pidstore.minters[current_app.config["DEPOSIT_PID_MINTER"]]
        id_ = id_ or uuid.uuid4()
    ...
    def edit(self, pid=None):
        ...
        with db.session.begin_nested():
            before_record_update.send(current_app._get_current_object(), record=self)
