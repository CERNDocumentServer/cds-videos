...
def test_record_unbuild_url(api_app):
    ...
    assert "1" == record_unbuild_url("https://cds.cern.ch/api/deposits/video/1")
...
def test_video_build_url(api_app):
    ...
    assert "https://cds.cern.ch/api/deposits/video/1" == video_build_url("1")
    assert (
        "https://cds.cern.ch/api/deposits/video/" "95b0716a-c726-4481-96fe-2aa02c72cd41"
...
def test_update_videos(api_project):
    ...
    assert project["videos"] == [{"$ref": video_1.ref}, {"$ref": new_ref_2}]
...
def test_add_video(
    api_app, es, users, location, project_deposit_metadata, video_deposit_metadata
):
    """Test add video."""
...
def test_project_delete_not_published(api_app, api_project, force):
    ...
    reclist = RecordMetadata.query.filter(
        RecordMetadata.id.in_([project_id, video_1_id, video_2_id])
    ).all()
...
@pytest.mark.parametrize("force", [False])  # , True])
def test_project_delete_one_video_published(api_app, api_project, force, users):
    """Test project delete when one video is published."""

    def check_project(
        number_of_videos, video_2_status, video_1_ref, video_2_ref, project_id
    ):
...
def test_project_publish_with_workflow(api_app, users, api_project, es, local_file):
    ...
    ) as mock_indexer, api_app.test_request_context(headers=headers, data=payload):
    ...
    expected = {"sse_simple_add": "SUCCESS", "sse_failing_task": "FAILURE"}
...
def test_project_partial_validation(
    api_app, db, deposit_metadata, location, video_deposit_metadata, users
):
