...
def test_simple_workflow(
    ...
    with api_app.test_client() as client:
        ...
        check_connection(
            [video_1],
            project_resolver.resolve(project_dict["metadata"]["_deposit"]["id"])[1],
        )
        ...
        assert file_1["id"]
        # check database: connection project <---> videos
        video_1 = deposit_video_resolver(video_1_dict["metadata"]["_deposit"]["id"])
        ...
        assert myfile["links"]["self"].startswith("https://localhost:5000/api/files/")
        ...
        video_2 = deposit_video_resolver(video_2_dict["metadata"]["_deposit"]["id"])
        ...
        # check database
        project = project_resolver.resolve(project_dict["metadata"]["_deposit"]["id"])[
            1
        ]
        ...
        assert res.status_code == 200
        project_dict = json.loads(res.data.decode("utf-8"))
        assert project_dict["metadata"]["title"]["title"] == "new project title"
        assert all(
            link.startswith("http://localhost/deposits/project")
            for (key, link) in project_dict["links"].items()
            if key not in ["html", "bucket"]
        ...
        assert project_before["publication_date"] == project["publication_date"]
...
def test_publish_project_check_indexed(
    ...
    with api_app.test_client() as client:
        ...
        res = client.get(
            url_for("invenio_deposit_rest.project_list"), headers=json_headers
        )
        ...
        with mock.patch("invenio_indexer.api.RecordIndexer.bulk_index") as mock_indexer:
            # [[ PUBLISH THE PROJECT ]]
            prepare_videos_for_publish([video_1, video_2])
            client.post(
                url_for(
            ...
            # get project record
            _, project_record = project_resolver.resolve(project_depid)[
                1
            ].fetch_published()
            # get video records
...
def test_boolean_fields_are_indexed(api_app, es, api_project, users, json_headers):
...
def test_project_keywords_serializer(
...
def test_project_access_rights_based_on_user_id(api_app, users, api_project):
    ...
    with api_app.test_client() as client:
        prepare_videos_for_publish([video_1, video_2])
        deposit_url = url_for("invenio_deposit_rest.project_item", pid_value=cds_depid)
        publish_url = url_for(
            "invenio_deposit_rest.project_actions",
            pid_value=cds_depid,
            action="publish",
        )
        # ensure that we are anonymous
        if current_user:
            logout_user()
...
def test_deleted(api_app, es, db, location, api_project, users, json_headers):
    ...
    with api_app.test_client() as client:
        ...
        res = client.get(
            url_for(
                "invenio_deposit_rest.project_list", q="_deposit.id:{0}".format(pid)
            ),
            headers=json_headers,
        )
        assert res.status_code == 200
...
def test_default_order(
    api_app, es, users, location, db, deposit_metadata, json_headers
):
    ...
    (project_1, _, _) = new_project(
        ...
        db,
        deposit_metadata,
        project_data={"title": {"title": "project 1"}},
    )
    project_1.commit()
    db.session.commit()
    (project_2, _, _) = new_project(
        api_app, users, db, deposit_metadata, project_data={"title": {"title": "alpha"}}
    )
    project_2.commit()
    db.session.commit()
    (project_3, _, _) = new_project(
        api_app, users, db, deposit_metadata, project_data={"title": {"title": "zeta"}}
    )
    project_3.commit()
    db.session.commit()
    (project_4, _, _) = new_project(
        ...
        db,
        deposit_metadata,
        project_data={"title": {"title": "project 2"}},
    )
    project_4.commit()
    db.session.commit()
    current_search_client.indices.refresh()
    ...
    def check_order(data, orders):
        hits = [hit["metadata"]["title"]["title"] for hit in data["hits"]["hits"]]
        assert hits == orders
    ...
    with api_app.test_client() as client:
        ...
        res = client.get(
            url_for("invenio_deposit_rest.project_list", sort="oldest_created"),
            headers=json_headers,
        )
        assert res.status_code == 200
        data = json.loads(res.data.decode("utf-8"))
...
@pytest.mark.skip(reason="Re-enable once we have exclusion on contributors role and id")
def test_search_excluded_fields(
    api_app, es, users, api_project, json_headers, location, project_deposit_metadata
):
    ...
    with api_app.test_client() as client:
        ...
        res = client.get(
            url_for("invenio_records_rest.recid_list", q="Director"),
            headers=json_headers,
        )
        assert res.status_code == 200
        data = json.loads(res.data.decode("utf-8"))
        ...
        res = client.get(
            url_for("invenio_records_rest.recid_list", q="67890"), headers=json_headers
        )
...
def test_aggregations(api_app, es, users, location, db, deposit_metadata, json_headers):
    ...
    with api_app.test_client() as client:
        ...
        res = client.get(
            url_for("invenio_deposit_rest.project_list", project_status="draft"),
            headers=json_headers,
        )
        assert res.status_code == 200
        data = json.loads(res.data.decode("utf-8"))
...
def test_sync_access_rights(
    ...
    def check_record_access_rights(recid):
        ...
        res = client.get(url_for("invenio_records_rest.recid_item", pid_value=recid))
        assert res.status_code == 200
        data = json.loads(res.data.decode("utf-8"))
    ...
    def check_video_access_rights(video_depid):
        # check video has same access rights of the project
        res = client.get(
            url_for("invenio_deposit_rest.video_item", pid_value=video_depid)
        )
        assert res.status_code == 200
        data = json.loads(res.data.decode("utf-8"))
        assert data["metadata"]["_access"] == access_rights
    ...
    def check_project_access_rights(project_depid):
        ...
        res = client.get(
            url_for("invenio_deposit_rest.project_item", pid_value=project_depid)
        )
        assert res.status_code == 200
        data = json.loads(res.data.decode("utf-8"))
        assert data["metadata"]["_access"] == access_rights
...
def test_project_edit_links(api_app, app, project_published, json_headers, users):
    ...
    with api_app.test_client() as client:
        url_video_1 = url_for(
            "invenio_records_rest.recid_item", pid_value=video_1["recid"]
        )
        url_project = url_for(
            "invenio_records_rest.recid_item", pid_value=project["recid"]
        )
