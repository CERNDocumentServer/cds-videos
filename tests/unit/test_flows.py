...
def test_avc_workflow_pass(
    api_app, db, api_project, access_token, json_headers, online_video, users
):
    """Test AVCWorkflow."""
    ...
    with api_app.test_request_context():
        url = url_for(
            "cds_flows.flow_feedback_item", flow_id=flow_id, receiver_id="avc"
        )
...
def test_avc_workflow_clean_video_transcode(
    ...
    for i, preset_quality in enumerate(presets, 1):
        ...
        flow.clean_task(
            flow=flow, task_name="file_transcode", preset_quality=preset_quality
        )
