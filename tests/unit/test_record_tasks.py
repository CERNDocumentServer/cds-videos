...
def test_get_keywords_from_api(cern_keywords):
    ...
    url = "test"
    return_value = type(url, (object,), {"text": json.dumps(cern_keywords)})
    with mock.patch("requests.get", return_value=return_value):
        keywords = _get_keywords_from_api(url)
        expected = {
            "751": dict(name="13 TeV", provenance=url),
            "856": dict(name="Accelerating News", provenance=url),
            "97": dict(name="accelerator", provenance=url),
            "14": dict(name="AEGIS", provenance=url),
        }
...
def test_update_existing_keywords(cern_keywords, location):
    ...
    # keyword harvested
    keywords_api = {
        "751": dict(name="13 TeV", provenance="http://home.cern/api/tags-json-feed"),
        "856": dict(
            name="Accelerating News", provenance="http://home.cern/api/tags-json-feed"
        ),
        "97": dict(
            name="accelerator", provenance="http://home.cern/api/tags-json-feed"
        ),
        "14": dict(name="AEGIS", provenance="http://home.cern/api/tags-json-feed"),
    }
    ...
    ks = {
        k.json["key_id"]: dict(name=k.json["name"], provenance=k.json["provenance"])
        for k in records
    }
...
def test_keyword_harvesting_one_time(db, cern_keywords, location):
    ...
    with mock.patch(
        "invenio_indexer.api.RecordIndexer.bulk_index"
    ) as mock_bulk_index, mock.patch("requests.get", return_value=return_value):
...
def test_keyword_harvesting_deleted_keywords(db, cern_keywords, location):
    ...
    # restore a key
    cern_keywords["tags"].append({"id": deleted["key_id"], "name": deleted["name"]})

    return_value = type("test", (object,), {"text": json.dumps(cern_keywords)})
    with mock.patch("invenio_indexer.api.RecordIndexer.bulk_index"), mock.patch(
